...
function Footer() {
  ...
  return (
    <div style={{ width: '100%', backgroundColor: theme.bgColor, color: theme.fontColor}} >
      <Container maxWidth="lg">
        <Stack spacing={2} py={5}>
          <Grid container spacing={2} >
            {/* Intro and Button */}
            <Grid item xs={12} sm={12} md={4} lg={4} xl={4}>
              <Stack spacing={2} alignItems={ isXs || isSm ? 'center' : 'flex-start'} >
                ...
                <Box width={{ xs: '260px', sm: '480px', md: '80%', lg: '100%', xl: '100%' }}>
                  <Typography color='inherit' >
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam.  
                  </Typography>
                </Box>
                <Button variant="contained" style={{ backgroundColor: theme.fontHoverColor, maxWidth: '60%' }} size="large" >Contact Us</Button>
              </Stack>
            ...
            <Grid item xs={12} sm={12} md={8} lg={8} xl={8}>
              { isXs ? (
              // Phone version
                <Stack direction="row" spacing={5} justifyContent={'center'} pt={2}>
                  {/* Left Column */}
                  <Stack direction="column" spacing={5}>
                    {/* Discover List */}
                    <Stack direction="column" spacing={2} >
                      <Typography variant="h6" fontWeight={'bold'} noWrap >Discover</Typography>
                      <Stack direction="column" spacing={1} >
                        {siteMap['Discover'].map(link => (
                          <Link href={link.link} key={link.label} underline="hover" color="inherit" sx={{ '&:hover': { color: theme.fontHoverColor } }}>
                            <Typography variant="body1" color='inherit' noWrap sx={{ '&:hover': { color: theme.fontHoverColor } }}>
                              {link.label}
                            </Typography>
                          </Link>
                        ))}
                    ...
                    {/* About Us List */}
                    <Stack direction="column" spacing={2} >
                      <Typography variant="h6" fontWeight={'bold'} noWrap >Learn More</Typography>
                      <Stack direction="column" spacing={1} >
                  ...
                  <Stack direction="column">
                    {/* Learn More List */}
                    <Stack direction="column" spacing={2} >
                        <Typography variant="h6" fontWeight={'bold'} noWrap >About Us</Typography>
                        <Stack direction="column" spacing={1} >
                ...
                // Desktop Version
                <Stack direction="row" spacing={10} justifyContent={'center'} p={{ sm: 2, xl: 0, lg: 0, md: 0}}>
                  {Object.keys(siteMap).map(section => (
                    <Stack key={section} direction="column" spacing={2} >
                      <Typography variant="h6" fontWeight={'bold'} noWrap >{section}</Typography>
                      <Stack direction="column" spacing={1} >
